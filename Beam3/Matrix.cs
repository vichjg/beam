...
namespace CSML
{
    public class Matrix
    {
        ...
        public Matrix(Complex[] values)
        {
			...
			/*if (values == null)
            {
				...
				for(int i = 0 ; i < rowCount ; i++) {
					...
					((ArrayList)Values[i]).Add(new Complex());
				}
			}*/
            ...
            {
                Values.Add(new ArrayList(1));
				if(!(values[i] is Complex)) {
					((ArrayList)Values[i]).Add(new Complex());
				}
                else { ((ArrayList)Values[i]).Add(values[i]);
				}
            }
        }
        ...
        public Matrix LUSafe()
        {
            ...
            for (int j = 1; j <= n; j++)
            {
                ...
                if (j < n)
                {
                    ...
                    if (this[j, j] == 0) {
						////
						this[j,j] = new Complex(0.000000001); }
        ...
        public override string ToString()
        {
			...
			for(int i = 1 ; i <= this.RowCount ; i++) {
				for(int j = 1 ; j <= this.ColumnCount ; j++) {
					texto = texto + "\t " + Convert.ToString(this[i,j].Re);
				}
				texto = texto + "\n";
			}
			texto = texto + "\n";
